...
public class TomActions : MonoBehaviour
{
    private Vector2 movementInput, cursorPosition;
    private Rigidbody2D rb;

    [SerializeField] private float moveSpeed = 5f;
    [SerializeField] private TomAnimator tomAnimator;
    ...
    {
        if (tomAnimator == null) return;

        movementInput = context.ReadValue<Vector2>();

        if (context.canceled) // If keydown is stopped, assign Last Input of X and Y to Idle animator
        {
            tomAnimator.StopMovement();
        }
        else
        {
            tomAnimator.UpdateMovement(movementInput);
        }
    }

    public void Block(InputAction.CallbackContext context)
    {
        Debug.Log("Block action called");
    }
